...
const DynamicForm = ({
    ...
    const getRemainingPercentage = () => Math.max(0, 100 - getTotalPercentage());
    ...
    const validateIngredients = () => {
        ...
        if (total > 100) {
            Alert.alert(t("FormulaError"), t("TotalCannotExceed", { total: total.toFixed(1) }));
            return { hasErrors: true, errors: {} };
        }

        if (autoCompletePercentage && total < 100 && total > 0) {
            return { hasErrors: false, errors: {}, autoComplete: true };
        }

        if (!autoCompletePercentage && total !== 100) {
            Alert.alert(
                t("IncompleteFormula"),
                t("AdjustPercentages", { total: total.toFixed(1) }),
        ...
        if (!hasErrors) {
            ...
            if (showIngredientsForm) {
                ...
                if (autoCompletePercentage && total < 100 && total > 0) {
                    ...
                    Alert.alert(
                        t("AutoCompleteFormula"),
                        t("AutoCompleteMessage", { total: total.toFixed(1), remaining: remaining.toFixed(1) }),
                        [
    ...
    const renderIngredientsForm = () => {
        ...
        return (
            <View style={styles.ingredientsSection}>
                {!!availableOils?.length > 0 && (
                    <View style={styles.ingredientsHeader}>
                        ...
                        <Text style={[
                            ...
                            totalPercentage > 100 && styles.percentageError
                        ]}>
                            {`${t("TotalPercentage")} %${totalPercentage.toFixed(2)}` }
                        </Text>
                    </View>
                )}
                ...
                {autoCompletePercentage && totalPercentage > 0 && totalPercentage < 100 && (
                    <View style={styles.remainingInfo}>
                        <Text style={styles.remainingText}>
                            {t("RemainingAutoComplete", { percent: remainingPercentage.toFixed(1) })}
                        </Text>
                    </View>
                )}
                ...
                {!autoCompletePercentage && totalPercentage > 0 && totalPercentage < 100 && (
                    <View style={styles.warningInfo}>
                        <Text style={styles.warningText}>
                            {t("WarningIncomplete", { total: totalPercentage.toFixed(1) })}
                        </Text>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    remainingInfo: { backgroundColor: "#f0f9ff", padding: 12, borderRadius: 8, marginTop: 12, borderWidth: 1, borderColor: "#0ea5e9" },
    remainingText: { fontSize: 14, color: "#0369a1", fontWeight: "500", textAlign: 'center' },
    warningInfo: { backgroundColor: "#fef3c7", padding: 12, borderRadius: 8, marginTop: 12, borderWidth: 1, borderColor: "#f59e0b" },
    warningText: { fontSize: 14, color: "#92400e", fontWeight: "500", textAlign: 'center' },
    successInfo: { backgroundColor: "#f0fdf4", padding: 12, borderRadius: 8, marginTop: 12, borderWidth: 1, borderColor: "#16a34a" },
    successText: { fontSize: 14, color: "#166534", fontWeight: "500", textAlign: 'center' },
